package pageUIs.nopcommerce.user;

public class UserLoginPageUI {
//	public static final String EMAIL_TEXTBOX = "//input[@id='Email']";
//	public static final String PASSWORD_TEXTBOX = "//input[@id='Password']";
//	public static final String LOGIN_BUTTON = "//button[contains(@class,'login-button')]";
//	public static final String EMAIL_ERROR_MESSAGE = "//span[@id='Email-error']";
//	public static final String UNSUCCESSFUL_MESSAGE = "//div[contains(@class,'validation-summary-errors')]";
//	public static final String MY_ACCOUNT_LINK = "//a[@class='ico-account']";
//	
	public static final String EMAIL_TEXTBOX = "id=Email";
	public static final String PASSWORD_TEXTBOX = "id=Password";
	public static final String LOGIN_BUTTON = "xpath=//button[contains(@class,'login-button')]";
	public static final String EMAIL_ERROR_MESSAGE = "id=Email-error";
	public static final String UNSUCCESSFUL_MESSAGE = "xpath=//div[contains(@class,'validation-summary-errors')]";
	public static final String MY_ACCOUNT_LINK = "class=ico-account";
	
}
